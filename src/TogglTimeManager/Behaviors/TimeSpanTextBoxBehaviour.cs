...
namespace TogglTimeManager.Behaviors
{
    /// <summary>
    ///     Exposes attached behaviors that can be
    ///     applied to Control objects.
    /// </summary>
    /// <remarks>Credits: https://www.codeproject.com/Articles/1148760/%2FArticles%2F1148760%2FDesign-for-WPF-MaskTextBox-to-Create-TimeSpan-Cont </remarks>
    public class TimeSpanTextBoxBehaviour
	{
		#region static
		...
		public static readonly DependencyProperty ValueProperty =
				DependencyProperty.RegisterAttached("Value", typeof(TimeSpan),
						typeof(TimeSpanTextBoxBehaviour), new UIPropertyMetadata(TimeSpan.Zero, OnValueChanged));
		...
		private static readonly DependencyProperty TimeTextBoxBehaviourProperty =
				DependencyProperty.RegisterAttached("TimeTextBoxBehaviour", typeof(TimeSpanTextBoxBehaviour),
						typeof(TimeSpanTextBoxBehaviour), new UIPropertyMetadata(null, OnValueChanged));
		...
		public TimeSpanTextBoxBehaviour(TextBox textBox)
		{
			_textBox = textBox;
		...
		private void OnPaste(object sender, DataObjectPastingEventArgs e)
		{
			...
			if (e.DataObject.GetDataPresent(DataFormats.Text))
			{
				try
				{
					TimeSpan currentTimeSpan = TimeSpanParse(Convert.ToString(e.DataObject.GetData(DataFormats.Text)).Trim(), _timerFormat == TimerFormats.Minutes);
		...
		private void OnPreviewKeyDown(object sender, KeyEventArgs e)
		{
		    switch (e.Key)
		    {
		        ...
		        case Key.Left:
		        {
		            int caret = _textBox.SelectionStart - 1;
		            if (caret < 0) return;
		...
		{
		    if (_textBox.Text.Length <= _textBox.SelectionStart) return;
		...
		private void ReplaceDigit(int position, char digit, bool normal)
		{
			...
			TimeSpan value = TimeSpanParse(firstPart, _timerFormat == TimerFormats.Minutes);
		...
		private void ChangeValueDependingOnLocation(int amount)
		{
			...
			int addValue = (_formatString[selectionStart] != _formatString[selectionStart + 1]
					...
					? 10 : 100) * amount;
			TimeSpan currentTimeSpan = TimeSpanParse(_textBox.Text, _timerFormat == TimerFormats.Minutes);
		...
		private void MaxTimeSpanChanged(TimeSpan timeSpan)
		{
			...
			TimeSpan valueTimeSpan = GetValue(_textBox);
		...
		{
			string hours = Math.Floor(timeSpan.TotalHours).ToString("000");
			string minutes = timeSpan.Minutes.ToString("00");
			string seconds = timeSpan.Seconds.ToString("00");
			string milliseconds = timeSpan.Milliseconds.ToString("000");
